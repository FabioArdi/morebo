...
def save_not_found_movies(movies: list):
    ...
    if len(movies) > 0:
        ...
        new_not_found = pd.DataFrame(movies, columns=['title'])
        if len(not_found) > 0:
...
if __name__ == "__main__":
    ...
    if not exists(movies_file): # create file if not exists
        fetched_movies = pd.DataFrame(columns=['movieId', 'title', 'genres', 'Title', 'Year', 'Rated', 'Released', 'Runtime', 'Genre', 'Director', 'Writer', 'Actors', 'Plot', 'Language', 'Country', 'Awards', 'Poster', 'Ratings', 'Metascore', 'imdbRating', 'imdbVotes', 'imdbID', 'Type', 'DVD', 'BoxOffice', 'Production', 'Website', 'Response'])
    ...
    if not exists(not_found_file): # create file if not exists
        not_found_movies = pd.DataFrame(columns=['title'])
    ...
    while response[0] == 200 and index < len(ml_movies): # fetch meta data for all movies til error occurs
        ...
        if ml_movie.title in fetched_movies['title'].values or ml_movie.title in not_found_movies['title'].values: # check if movie is already fetched
            print(f"Already fetched meta data for '{ml_movie.title}'")
            index += 1
        else: # fetch meta data for movie
            print(f"Fetching meta data for '{ml_movie.title}'")
            movie_title = ml_movie.title.replace(" ", "+") # replace spaces with + for url
            # url = f"https://www.omdbapi.com/?t={movie_title}&apikey=aa348489" # API key with school mail
            url = f"https://www.omdbapi.com/?t={movie_title}&apikey=967eb8c6" # API key with private mail
            ...
            if not response[0] == 200: # check if response is ok
                ...
                print_error(response, ml_movie)
                new_not_found.append(ml_movie.title)
            else: # save response
